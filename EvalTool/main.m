...
CamMapPath = '../ForEval/';   % Put model results in this folder.
%Models = {'2017-CVPR-FPN','2017-CVPR-MaskRCNN','2017-CVPR-PSPNet','2018-CVPR-PiCANet','2018-DLMIA-UNet++','2019-CVIU-ANet_SRM' ,'2019-CVPR-BASNet','2019-CVPR-CPD_ResNet','2019-CVPR-HTC','2019-CVPR-MSRCNN','2019-CVPR-PFANet','2019-CVPR-PoolNet' ,'2019-ICCV-EGNet'};   % You can add other model like this format: Models = {'2019-ICCV-EGNet','2019-CVPR-CPD'};
Models = {'PINet_Origin','PINet_New'}
modelNum = length(Models);

% ---- 2. Ground-truth Datasets Setting ----
DataPath = '../Dataset/Old_Camouflage/TestDataset/';
Datasets = {'CHAMELEON','CAMO','COD10K'};  % You may also need other datasets, such as Datasets = {'CAMO','CPD1K'};
%Datasets = {'COD10K'}

% --- for translate direction
%Direction = {'Up','Down','Right','Left'};
%DirNum = length(Direction);
...
for d = 1:datasetNum
    ...
    for m = 1:modelNum
        ...
        threshold_Fmeasure = gpuArray(threshold_Fmeasure);
        threshold_Emeasure = gpuArray(threshold_Emeasure);    
        threshold_Precion = gpuArray(threshold_Precion);  
        threshold_Recall = gpuArray(threshold_Recall);  
        Smeasure = gpuArray(Smeasure);  
        wFmeasure = gpuArray(wFmeasure);  
        adpFmeasure = gpuArray(adpFmeasure);  
        adpEmeasure = gpuArray(adpEmeasure);  
        MAE = gpuArray(MAE);  
        ...
        fprintf('(Dataset:%s; Model:%s) Smeasure:%.3f; meanEm:%.3f; wFmeasure:%.3f; MAE:%.3f;\n',dataset,model,Sm, meanEm, wFm, mae);
